...
const localActivity: Msg[] = [
  { id: "local-1", name: "SYSTEM", text: "Private terminal connected.", tag: "system" },
  { id: "local-2", name: "TORION", text: "Risk layer monitoring allocation.", tag: "system" },
...
export function TerminalChat({ events = [], userName = "User" }: Props) {
  ...
  async function sendMessage() {
    ...
    try {
      ...
      console.warn("Activity send failed:", error);
    } finally {
      setSending(false);
  ...
  const feed = [...localActivity, ...eventMessages, ...messages].slice(-12);
    ...
    <section className="relative overflow-hidden rounded-[24px] bg-[#101114] p-4 ring-1 ring-white/5">
      ...
      <div className="relative z-10">
        <div className="mb-3 flex items-center justify-between">
          <div>
            <p className="text-xs text-white/40">Private room</p>
        ...
        <div className="mt-3 flex items-center gap-2 rounded-full bg-black/30 p-2 ring-1 ring-white/5">
          <input
            ...
            placeholder="Send activity message..."
